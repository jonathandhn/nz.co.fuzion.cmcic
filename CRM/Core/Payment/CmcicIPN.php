...
class CRM_Core_Payment_CmcicIPN
{
  ...
  */
   ...
   * @return string
   */
    public function cmcic_receipt($mac_ok)
    {
        return "version=2\ncdr=" . ($mac_ok ? '0' : '1') . "\n";
    }
   ...
   */
    public function cmcic_receipt_exit($mac_ok)
    {
   ...
   * @return boolean
   */
    public function cmcic_validate_response()
    {
   ...
   */
    public function main($paymentProcessor)
    {
      ...
      // based on the contribution id
        ...
        if (!$this->isKnownResultCode($resultCode)) {
            \Civi::log()->error(
                "Monetico IPN received an unknown code-retour '{$resultCode}' "
                . "for contribution #{$contributionID}."
            );
            $this->cmcic_receipt_exit(false);
            return false;
        }
        ...
        if ($resultCode === 'payetest' && empty($paymentProcessor['is_test'])) {
            \Civi::log()->error(
                "Monetico production IPN received sandbox code payetest for contribution #{$contributionID}."
            );
            $this->cmcic_receipt_exit(false);
            return false;
        }

      // Fetch the contribution state and financial details for idempotency and amount validation.
      ...
      // Idempotency: If the contribution is already Completed, verify that it is the exact same transaction replayed
        if ($currentStatus === 'Completed') {
            ...
            if ($isSameTransaction) {
                \Civi::log()->debug(
                    "Monetico IPN received for already completed contribution #{$contributionID} "
                    . '(same transaction). Returning idempotent receipt.'
                );
            ...
            \Civi::log()->error(
                "Monetico IPN received second distinct transaction for already completed contribution "
                . "#{$contributionID}. Rejecting."
            );
            $this->cmcic_receipt_exit(false);
            return false;
        }
      ...
      // Strict Validation of IPN amount & currency format against the CiviCRM contribution
        ...
        if (!preg_match('/^([0-9]+(?:\.[0-9]{1,2})?)([A-Z]{3})$/', $montantBrut, $matches)) {
            \Civi::log()->error(
                "Monetico IPN malformed montant format for contribution #{$contributionID}: '{$montantBrut}'"
            );
            $this->cmcic_receipt_exit(false);
            return false;
        }
      ...
      // Reject fractional amounts for zero-decimal currencies (e.g. 10.25JPY is invalid)
        if ($this->isZeroDecimalCurrency($receivedCurrency) && str_contains($rawAmountString, '.')) {
            \Civi::log()->error(
                "Monetico IPN invalid decimal fraction for zero-decimal currency {$receivedCurrency} "
                . "on contribution #{$contributionID}: '{$montantBrut}'"
            );
            $this->cmcic_receipt_exit(false);
            return false;
        }
        ...
        if ($receivedUnits !== $expectedUnits || $receivedCurrency !== $expectedCurrency) {
            \Civi::log()->error(sprintf(
                'Monetico IPN amount/currency mismatch for contribution #%d: '
                . 'received %d units (%s), expected %d units (%s).',
                $contributionID,
                $receivedUnits,
                $receivedCurrency,
                $expectedUnits,
   ...
   */
    public function processFailedTransaction($contributionID, $currentStatus = '')
    {
        if ($currentStatus === 'Completed') {
            \Civi::log()->warning(
                "Monetico IPN received cancellation for already completed contribution #{$contributionID}. Ignoring."
            );
            return;
        }
   ...
   */
    protected function isZeroDecimalCurrency(string $currency): bool
    {
        ...
        'JPY', 'KRW', 'CLP', 'PYG', 'UGX', 'VND', 'BIF', 'DJF',
        'GNF', 'KMF', 'MGA', 'RWF', 'VUV', 'XAF', 'XOF', 'XPF',
        ];

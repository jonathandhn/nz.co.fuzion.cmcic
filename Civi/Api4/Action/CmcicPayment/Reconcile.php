...
class Reconcile extends AbstractAction
{
   ...
   */
    public static function determineDiscrepancyAction(
        string $currentStatus,
        string $bankState,
        float $recreditsTotal,
   ...
   * @return array
   */
    public static function buildRefundPaymentValues(
        int $contributionId,
        float $totalAmount,
        string $currency,
        int $processorId,
    ...
    public function _run(Result $result): void
    {
        ...
        if (!$contributionId && !empty($this->trxnId)) {
            ...
            if (count($payments) > 1) {
                throw new CRM_Core_Exception(
                    "Multiple Monetico payments found matching trxn_id '{$this->trxnId}'. "
                    . 'Reconciliation requires an explicit contribution_id.'
                );
            }
            if (count($payments) === 1) {
        ...
        if (!$contributionId) {
            throw new CRM_Core_Exception(
                'Contribution ID or valid trxn_id for a Monetico payment is required for reconciliation.'
            );
        }
        ...
        if (count($payments) > 1) {
            throw new CRM_Core_Exception(
                "Multiple Monetico payment entries found for contribution #{$contributionId}."
            );
        }
        if (count($payments) === 0) {
            throw new CRM_Core_Exception(
                "Contribution #{$contributionId} has no associated Monetico payment processor entry."
            );
        }
        ...
        if (!($initialProcessor instanceof \CRM_Core_Payment_Cmcic)) {
            throw new CRM_Core_Exception(
                "Payment processor #{$processorId} is not a Monetico Payment_Cmcic processor."
            );
        }
